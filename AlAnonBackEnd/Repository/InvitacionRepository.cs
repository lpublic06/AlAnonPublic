...
namespace AlAnon.Repository
{
	public class InvitacionRepository : IInvitacionRepository
	{
		...
		public async Task<RespuestaDto<List<InvitacionDto>>> ObtenerInvitacionesActuales(DateTime today)
		{
			var respuesta = new RespuestaDto<List<InvitacionDto>>();
			try
			{
		...
		public async Task<RespuestaDto<List<InvitacionDto>>> ObtenerInvitacionesPasadas(DateTime today)
		{
			var respuesta = new RespuestaDto<List<InvitacionDto>>();
			try
			{
		...
		public async Task<RespuestaDto<List<InvitacionDto>>> ObtenerInvitacionesDelMes(DateTime firstDayOfMonth)
		{
			...
			try
			{
				...
				respuesta.Data = _mapper.Map<IEnumerable<Invitacion>, IEnumerable<InvitacionDto>>(_db.Invitaciones.Where(r => r.FechaEntera >= firstDayOfMonth && r.FechaEntera <= lastDayOfMonth).OrderBy(r => r.FechaEntera)).ToList();
			}
			catch (Exception ex)
			{
				throw ex;
		...
		public async Task<RespuestaDto<List<InvitacionDto>>> ObtenerInvitacionesDelaSemana(DateTime today)
		{
			var respuesta = new RespuestaDto<List<InvitacionDto>>();
			try
			{
